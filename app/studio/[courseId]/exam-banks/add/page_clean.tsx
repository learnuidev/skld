...
function QuestionEditorCard({
  ...
  const handleCorrectOptionChange = (optionIndex: number) => {
    if (question.type === "SINGLE_SELECT_MULTIPLE_CHOICE") {
      ...
      const newIndexes = exists
        ...
        : [...current, optionIndex];
      onUpdate("correctOptionIndexes", newIndexes.length > 0 ? newIndexes : undefined);
  ...
  const isOptionCorrect = (optionIndex: number) => {
    if (question.type === "SINGLE_SELECT_MULTIPLE_CHOICE" || question.type === "TRUE_FALSE") {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-xl transition-shadow p-8">
          ...
          {question.options.map((option, oi) => (
            <div key={oi} className="flex gap-3 items-start">
              ...
              <input
                ...
                value={option}
                onChange={(e) => onOptionUpdate(oi, e.target.value)}
                className="flex-1 px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-slate-500 dark:focus:ring-slate-400"
                placeholder={`Option ${String.fromCharCode(65 + oi)}`}
              />
              <Button
                variant="ghost"
